...
class Render:
	...
	# The global frame counter.  Used to time some effects (like flashing effects)
	frameCount = 0
	
	def initGraphics(self, w):
		self.windowSurfaceObj = w
		self.redColor = pygame.Color(255, 0, 0)
		self.whiteColor = pygame.Color(255, 255, 255)
		self.humanNameColor = pygame.Color(128, 255, 122)
	...
	def setInitialRenderState(self):
		self.changeRenderState(RENDER_STATE_COMPANY_LOGO)
		
	def changeRenderState(self, newState):
		if newState == RENDER_STATE_COMPANY_LOGO:
			self.currentRenderState = RENDER_STATE_COMPANY_LOGO
		...
		if newState == RENDER_STATE_TITLE_SCREEN:
			self.currentRenderState = RENDER_STATE_TITLE_SCREEN
	...
	def updateLogic(self):
		if self.currentRenderState == RENDER_STATE_COMPANY_LOGO:
			return
		if self.currentRenderState == RENDER_STATE_TITLE_SCREEN:
		...
		if self.currentRenderState == RENDER_STATE_COMPANY_LOGO:
			...
			if self.waitingToChangeToTitleScreenState == True:
				if (ticks - self.waitingToChangeToTitleScreenStateTime) >= RENDER_LOGO_WAIT_BEFORE_CHANGING_TO_TITLE_SCREEN_TIME:
					self.changeRenderState(RENDER_STATE_TITLE_SCREEN)
				return
